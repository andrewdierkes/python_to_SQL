...
df_strip.insert(3, 'Intra_Strip_CV_n=5', round(((df_strip['Intra Strip StDev']/df_strip['Intra Strip Average RFU'])*100),4))
...
df_strip_cv_data['average_intrastrip_cv_n3'] = df_strip_cv_avg
...
#Compare Channels of similar position
...
#Strip Repeat Data for individual Channels

#For CH1400
df_stripname_ch1400 = slice_name(df_ch1400, 0,16,13)
strip_ch1400 = unique(df_stripname_ch1400.iloc[:,0],3)

strip_ch1400_avg, strip_ch_1400_stdev = chunk_cv(df_ch1400['Max RFU'], 3)



strip_ch1400_dict = {}
strip_ch1400_dict['ch1400_strip_repeat_name'] = strip_ch1400
strip_ch1400_dict['ch1400_repeat_average'] = strip_ch1400_avg
strip_ch1400_dict['ch1400_repeat_stdev'] = strip_ch_1400_stdev
#print(strip_ch1400_dict)
df_strip_ch1400 = pd.DataFrame(strip_ch1400_dict)
df_strip_ch1400['ch1400_strip_repeat_cv'] = round((df_strip_ch1400.iloc[:,2]/df_strip_ch1400.iloc[:,1])*100, 3)
#display(df_strip_ch1400)

#For CH2200
df_stripname_ch2200 = slice_name(df_ch2200, 0,16,13)
strip_ch2200 = unique(df_stripname_ch2200.iloc[:,0],3)

strip_ch2200_avg, strip_ch_2200_stdev = chunk_cv(df_ch2200['Max RFU'], 3)



strip_ch2200_dict = {}
strip_ch2200_dict['ch2200_strip_repeat_name'] = strip_ch2200
strip_ch2200_dict['ch2200_repeat_average'] = strip_ch2200_avg
strip_ch2200_dict['ch2200_repeat_stdev'] = strip_ch_2200_stdev

df_strip_ch2200 = pd.DataFrame(strip_ch2200_dict)
df_strip_ch2200['ch2200_strip_repeat_cv'] = round((df_strip_ch2200.iloc[:,2]/df_strip_ch2200.iloc[:,1])*100, 3)
#display(df_strip_ch2200)

#For CH3000
df_stripname_ch3000 = slice_name(df_ch3000, 0,16,13)
strip_ch3000 = unique(df_stripname_ch3000.iloc[:,0],3)

strip_ch3000_avg, strip_ch_3000_stdev = chunk_cv(df_ch3000['Max RFU'], 3)



strip_ch3000_dict = {}
strip_ch3000_dict['ch3000_strip_repeat_name'] = strip_ch3000
strip_ch3000_dict['ch3000_repeat_average'] = strip_ch3000_avg
strip_ch3000_dict['ch3000_repeat_stdev'] = strip_ch_3000_stdev

df_strip_ch3000 = pd.DataFrame(strip_ch3000_dict)
df_strip_ch3000['ch3000_strip_repeat_cv'] = round((df_strip_ch3000.iloc[:,2]/df_strip_ch3000.iloc[:,1])*100, 3)
#display(df_strip_ch3000)

#For CH3800
df_stripname_ch3800 = slice_name(df_ch3800, 0,16,13)
strip_ch3800 = unique(df_stripname_ch3800.iloc[:,0],3)

strip_ch3800_avg, strip_ch_3800_stdev = chunk_cv(df_ch3800['Max RFU'], 3)



strip_ch3800_dict = {}
strip_ch3800_dict['ch3800_strip_repeat_name'] = strip_ch3800
strip_ch3800_dict['ch3800_repeat_average'] = strip_ch3800_avg
strip_ch3800_dict['ch3800_repeat_stdev'] = strip_ch_3800_stdev

df_strip_ch3800 = pd.DataFrame(strip_ch3800_dict)
df_strip_ch3800['ch3800_strip_repeat_cv'] = round((df_strip_ch3800.iloc[:,2]/df_strip_ch3800.iloc[:,1])*100, 3)
#display(df_strip_ch3800)
...
df_channels = pd.concat(
                         [df_strip_ch1400.reset_index(drop=True), 
                         df_strip_ch2200.reset_index(drop=True), 
                         df_strip_ch3000.reset_index(drop=True), 
                         df_strip_ch3800.reset_index(drop=True), 
                         df_intrastrip_avg.reset_index(drop=True)], 
                         axis=1
                       )

df_channels.drop(df_channels.columns[[4,8,12]],axis=1, inplace=True)

df_channels.rename(
                    columns={'ch1400_strip_repeat_name':'strip_repeat_name'}, 
                    inplace=True
                  )
...
#find the average rfu for each assay and add to df
ch_1400=df_channels.iloc[:,1]
ch_2200=df_channels.iloc[:,4]
ch_3000=df_channels.iloc[:,7]
ch_3800=df_channels.iloc[:,10]

avg_list = []
for var in list(zip(ch_1400,ch_2200,ch_3000,ch_3800)):
    avg_rfu = (sum(var)/4)
    avg_list.append(avg_rfu)
    
df_avg = pd.DataFrame(avg_list, columns=['average_rfu'])


df_channel_final = pd.concat([df_channels.reset_index(drop=True), df_avg.reset_index(drop=True)], axis=1)
display(df_channel_final)
...
conn_string = 'postgresql://postgres:lumiradb@10.0.3.164:5432/metabolics'
db = create_engine(conn_string)
conn = db.connect()

df_channel_final.to_sql(f'channelanalysis_{_date}', con=conn)
